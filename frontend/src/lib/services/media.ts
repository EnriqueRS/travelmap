...
export const mediaService = {
  ...
  uploadLocalPhoto: async (tripId: string, file: File): Promise<AppPhoto> => {
    ...
    const res = await axios.post(`${API_URL}/media/trips/${tripId}/photos`, formData, {
      headers: {
  ...
  linkExternalPhoto: async (tripId: string, url: string, externalId: string): Promise<AppPhoto> => {
    const token = getToken()
    const res = await axios.post(`${API_URL}/media/trips/${tripId}/photos`, {
      provider: 'immich',
      url,
      externalId,
      showOnMap: false
  ...
  updatePhoto: async (photoId: string, data: { showOnMap?: boolean, isCover?: boolean }): Promise<AppPhoto> => {
    const token = getToken()
