...
- **Multi-Select Photos with Batch Operations**: Added the ability to select multiple photos using Shift+click (range select) and Ctrl/Cmd+click (toggle individual). Batch operations include: Show on Map, Hide from Map, Hide from Gallery, Show in Gallery, Batch GPS, and Delete. Includes Select All/Deselect All and a mobile-optimized toolbar.
...
- **Selección Múltiple de Fotos con Operaciones en Lote**: Se añadió la capacidad de seleccionar múltiples fotos usando Shift+clic (selección de rango) y Ctrl/Cmd+clic (alternar individual). Las operaciones en lote incluyen: Mostrar en Mapa, Ocultar del Mapa, Ocultar de Galería, Mostrar en Galería, GPS en Lote y Eliminar. Incluye Seleccionar Todas/Deseleccionar Todas y una barra de herramientas optimizada para móvil.
